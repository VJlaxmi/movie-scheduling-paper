...
(ii)~a bi-level matheuristic where NSGA-II evolves block permutations at the upper level while a global greedy decoder schedules scenes in block-guided order, simultaneously optimizing cost and makespan;
...
Computational experiments on synthetic instances with 10--25 scenes show that BDM-AOS achieves cost within 1.6\% of standalone GA on small instances (S10) and within 0.4\% of PSO on larger instances (S25), while providing a Pareto front of cost-makespan trade-off solutions unavailable from single-objective methods.
The decomposition approach demonstrates zero variance across seeds on multiple instance sizes, indicating robust structure-guided search.
