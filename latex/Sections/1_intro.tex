...
\begin{enumerate}
    ...
    \item \textbf{Bi-level Matheuristic:} The upper level uses NSGA-II~\citep{deb2002} to evolve permutations of shooting blocks, simultaneously optimizing total cost and makespan. The lower level solves each block as a small MILP sub-problem or via a constraint-aware greedy heuristic, providing optimal or near-optimal intra-block scheduling. This is the first matheuristic approach for the MSSP.
    ...
    \item \textbf{Rigorous experimental evaluation:} Comprehensive experiments on 5 instance sizes (10--30 scenes) with 5 independent seeds per algorithm, including ablation studies and Wilcoxon rank-sum statistical testing.
\end{enumerate}
