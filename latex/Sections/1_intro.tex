...
The \emph{Movie Scene Scheduling Problem} (MSSP) determines the order and day assignments of scenes to minimize total production cost---comprising actor wages, hierarchical location transfer costs, deadline penalties, criticality-weighted resource utilization, and actor holding fees---subject to a rich set of real-world constraints: actor availability calendars, a three-tier geographic location hierarchy (city--town--location), crew travel logistics that restrict town changes across consecutive days, daily shooting capacity, precedence relations, time windows, labor regulations limiting consecutive working days, specialized equipment sharing, mandatory scene co-scheduling groups, minimum scene separation requirements, location concurrency capacities, and lead-actor rest period requirements.
...
\begin{enumerate}
    \item \textbf{Comprehensive MILP formulation:} Building on our prior single-objective scheduling work~\citep{vjlaxmi2024}, we present the most complete MSSP formulation to date, with \textbf{19 constraint families} over a three-tier geographic hierarchy (city--town--location): the 13-family baseline extended with equipment sharing (C14), actor booking/holding fees (C15), mandatory co-scheduling (C16), minimum scene separation (C17), location concurrency (C18), and lead-actor rest periods (C19).
    This substantially extends prior formulations~\citep{liu2019, tekin2023} with operational constraints absent from all existing models.
    ...
    \item \textbf{Bi-level Decomposition with Greedy Reassembly:} The upper level uses NSGA-II~\citep{deb2002} to evolve permutations of shooting blocks, simultaneously optimizing total cost and makespan. The lower level applies a global greedy decoder that packs scenes into days in block-guided order, enforcing all 19 constraint families. This is the first decomposition-based approach for the MSSP.
    ...
    We include a detailed MILP-vs.-BDM-AOS comparison (Section~\ref{sec:milp_comparison}) reporting both objective costs and runtimes, showing BDM-AOS matches MILP optimality on small instances with up to $186\times$ speedup, and remains the only feasible approach for $n \geq 30$.
\end{enumerate}
