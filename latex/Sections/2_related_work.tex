...
Our formulation (Section~\ref{sec:formulation}) addresses this gap with a three-tier city--town--location hierarchy, linking variables that couple scenes with actor participation and location usage, town-level logistics constraints, criticality-weighted utilization, and actor labor regulations---producing 13 structured constraint families that capture operational realities absent from prior models.
...
BDM-AOS is the first MSSP approach combining (i)~a structurally rich MILP formulation with hierarchical location modeling and geographic logistics constraints, (ii)~decomposition via graph clustering, (iii)~a bi-level metaheuristic-decoder framework enforcing all 13 constraint families, (iv)~multi-objective optimization, and (v)~adaptive operator selection with problem-specific operators.
